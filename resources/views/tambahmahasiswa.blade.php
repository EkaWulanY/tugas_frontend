  ...
  <main class="p-6">
    <div class="bg-white p-8 rounded-xl shadow-lg max-w-2xl mx-auto">
      <h2 class="text-2xl font-semibold mb-6 text-cyan-700 flex items-center gap-2">
        <i data-lucide="user-plus" class="w-6 h-6"></i> Tambah Data Mahasiswa
      </h2>
      ...
      <form action="{{ route('mahasiswa.store') }}" method="POST" class="space-y-5">
        ...
        <div>
          <label class="block text-sm font-medium text-gray-700 mb-1">Judul Skirpsi</label>
          <input type="judul_skripsi" name="judul_skripsi" required
            class="w-full px-4 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-cyan-500">
        </div>
